...
This repository currently supports the Phase 0 frameworkless PHP foundation on the latest `main` revision. Security fixes should target that architecture; Phase 1 normalization and framework migrations are out of scope for P0 reports.
...
- [ ] Confirm the Apache deny rules or equivalent Nginx rules block `.env`, dotfiles, `includes/`, `database/`, `scripts/`, `storage/`, `tests/`, and `deploy/`.
...
- [ ] Run PHP lint, static tests, integration tests, and HTTP smoke tests against a disposable database.
